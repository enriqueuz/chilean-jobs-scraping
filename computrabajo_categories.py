...
def scrape(driver):
    ...
    categories = driver.find_element_by_xpath('/html/body/main/section/section/div[2]/ul').find_elements_by_tag_name('a')
    categories_list = []

    for cat in categories:
        # category_dict = {
        #     cat.text: cat.get_attribute('href')
        # }
        category_tuple = (cat.text, cat.get_attribute('href'))
        categories_list.append(category_tuple)
    
    start_date = datetime.now().date()

    # When resuming script, remove categories already scrappy from categories list
    if resume_category:
        for i in range(0, len(categories_list)):       
            if categories_list[i][0] == resume_category:
                categories_list = categories_list[i:]
                break
    ...
    for category in categories_list:
        ...
        print(f'Scraping {category[0]} jobs')

        # while total_num_jobs > jobs_scrapped and pages_scrapped < 2:
        while total_num_jobs > jobs_scrapped:

            if pages_scrapped >= 30:
                break
            ...
            if resume_page:
                print(f'Reaching last page scrapped in category {resume_category}...')
                while resume_page != current_page:
                    next_page = driver.find_element_by_link_text('Siguiente')
                    next_page.click()
                    current_page = driver.find_element_by_class_name(
                ...
                pages_scrapped += current_page # Update pages scrapped variable
                resume_page = False
            ...
            for i in range(num_jobs_in_page):
                ...
                number_of_job = i + 1
                print(f'Scraping job {number_of_job} in page {current_page}')
